...
router.post('/save-design', verifyToken, async (req, res) => {
  try {
    ...
    // New public IDs are always generated by the server. A client-supplied id
    // may identify an existing design, but it never reserves a new namespace.
    ...
    // Determine whether the requested id is an owned update or a template/copy.
    if (existingId) {
    ...
    // Enforce the product's one-card-per-member invariant regardless of whether
    // the client supplied ?id=. This closes the bypass where arbitrary ids
    // created additional cards.
    if (!isUpdate && ownerId) {
    ...
    // Defense in depth for legacy/data-migration states where more than one card
    // may already exist. A client-provided id can never skip this creation limit.
    ...
    const checkNames = [
      ...
      normalizeName(data.inputs?.['input-name_en']),
      normalizeName(data.inputs?.['input-name']),
