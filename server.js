...
const backgroundsCollectionName =
  process.env.MONGO_BACKGROUNDS_COLL || "backgrounds";
let db;
...
MongoClient.connect(mongoUrl)
  ...
  .catch((err) => {
    ...
    console.warn('Proceeding without DB connection; some features may be unavailable.');
    db = null;
  });
...
function sanitizeInputs(inputs) {
  ...
  if (sanitized.dynamic && sanitized.dynamic.phones) {
    sanitized.dynamic.phones = sanitized.dynamic.phones.map((phone) => ({
      ...
      // التأكد من أن القيمة موجودة قبل التعقيم
      value:
        phone && phone.value ? DOMPurify.sanitize(String(phone.value)) : "",
    }));
  }
  return sanitized;
}
...
app.get(["/nfc/viewer", "/nfc/viewer.html"], async (req, res) => {
  try {
    ...
    if (!id || id === "undefined") {
      // التحقق من عدم وجود ID
      res.setHeader("X-Robots-Tag", "noindex, noarchive");
      return res
        .status(400)
        .send("Card ID is missing. Please provide an ?id= parameter.");
    }

    // --- باقي الكود منسوخ من المسار القديم ---
    const doc = await db
      .collection(designsCollectionName)
      .findOne({ shortId: id });

    if (!doc || !doc.data) {
      // التحقق من وجود doc.data
      res.setHeader("X-Robots-Tag", "noindex, noarchive");
    ...
    db.collection(designsCollectionName)
      .updateOne({ shortId: id }, { $inc: { views: 1 } })
      .catch((err) =>
        console.error(`Failed to increment view count for ${id}:`, err),
      );
  ...
  if (err instanceof multer.MulterError) {
    if (err.code === "LIMIT_FILE_SIZE") {
      return res
        .status(400)
        .json({
          error: `حجم الملف كبير جدًا. الحد الأقصى ${err.field ? err.field : "10"} ميجابايت.`,
        });
    }
    ...
    return res
      .status(400)
      .json({ error: err.message || "خطأ غير معروف أثناء الرفع." });
  }
  next();
}
...
app.post("/api/save-design", async (req, res) => {
  try {
    ...
    if (data.dynamic) {
      ...
      if (data.dynamic.social) {
        data.dynamic.social = data.dynamic.social.map((link) => ({
          ...link,
          value:
            link && link.value ? DOMPurify.sanitize(String(link.value)) : "",
        }));
      }
      if (data.dynamic.staticSocial) {
        for (const key in data.dynamic.staticSocial) {
          if (
            data.dynamic.staticSocial[key] &&
            data.dynamic.staticSocial[key].value
          ) {
            data.dynamic.staticSocial[key].value = DOMPurify.sanitize(
              String(data.dynamic.staticSocial[key].value),
            );
          }
        }
      }
    ...
    await db
      .collection(designsCollectionName)
      .insertOne({ shortId, data, createdAt: new Date(), views: 0 });
...
app.get("/api/get-design/:id", async (req, res) => {
  try {
    ...
    if (!doc || !doc.data)
      return res
        .status(404)
        .json({ error: "Design not found or data missing" });
  ...
  try {
    ...
    if (doc.url) {
      try {
        ...
        console.error(
          `Error deleting file for background ${shortId}:`,
          fileError,
        );
      }
    }
