...
const env = cleanEnv(process.env, {
    ...
    MONGO_URI: str({ default: '' }), // We will check this in server.js instead of crashing immediately
    MONGO_DB: str({ default: 'nfc_db' }),
    MONGO_DESIGNS_COLL: str({ default: 'designs' }),
    MONGO_BACKGROUNDS_COLL: str({ default: 'backgrounds' }),
    JWT_SECRET: str({ default: 'fallback_secret_for_local_dev' }),
    CLOUDINARY_CLOUD_NAME: str({ default: '' }), // Optional per README but kept for structure
    CLOUDINARY_API_KEY: str({ default: '' }),
    CLOUDINARY_API_SECRET: str({ default: '' }),
    PUBLIC_BASE_URL: str({ default: 'https://mcprim.com' }), // changed from url() to str() with default
    SITE_BASE_URL: str({ default: '' }), // Sometimes used in server.js instead of PUBLIC_BASE_URL
