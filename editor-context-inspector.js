/**
 * MC PRIME NFC — Context Inspector v1.0
...
(function (global) {
    ...
    function getDescriptor(element) {
        var key = [element.id, element.className, element.getAttribute('data-element-type'), element.getAttribute('aria-label')]
            .filter(Boolean).join(' ');
        var descriptor = elementMap.find(function (entry) { return entry.match.test(key); });
        return descriptor || {
            labelAr: element.getAttribute('aria-label') || element.id || 'عنصر',
            labelEn: element.getAttribute('aria-label') || element.id || 'Element',
            controls: []
        };
    }
    ...
    function openAdvancedControls() {
        ...
        if (target) {
            ...
            if (global.EditorTabs && typeof global.EditorTabs.activate === 'function') {
                global.EditorTabs.activate(target.closest('#panel-elements') ? 'tab-content' : 'tab-design');
            }
    ...
    function injectStyles() {
        ...
        style.textContent = [
            ...
            '.editor-selected-element{outline:2px solid var(--accent-primary,#4da6ff)!important;outline-offset:3px!important}.editor-inspector-highlight{animation:eciPulse .8s ease 2}@keyframes eciPulse{50%{box-shadow:0 0 0 4px rgba(77,166,255,.25)}}',
            '@media(max-width:1024px){.editor-context-inspector{display:none}}'
        ].join('');
