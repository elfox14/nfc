...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function enhanceThumbnail(thumbnailContainer, existingImgSrc, viewUrl, editUrl) {
    if (!thumbnailContainer) return null;

    // If already enhanced, skip
    if (thumbnailContainer.dataset.enhanced === '1') return thumbnailContainer.querySelector('.thumb-bg-layer');
    ...
    if (existingImgSrc) {
      ...
      if (thumbnailContainer.dataset.bg) bgLayer.dataset.bg = thumbnailContainer.dataset.bg;
    }
    ...
    btnView.innerHTML = '<i class="fas fa-eye" aria-hidden="true"></i>&nbsp;عرض';
    ...
    btnEdit.innerHTML = '<i class="fas fa-pencil-alt" aria-hidden="true"></i>&nbsp;قالب';
  ...
  function enhanceExistingCards() {
    ...
    cards.forEach(card => {
      ...
      const possibleEdit = card.querySelector('a.edit-link, a.edit, a.use-template');
      if (possibleEdit && possibleEdit.href) editUrl = possibleEdit.href;

      // if thumbnail contains <img>, use its src
      const img = thumbnail.querySelector('img');
      const imgSrc = img ? (img.dataset.src || img.getAttribute('src') || '') : '';
  ...
  if (galleryContainer) {
    const mo = new MutationObserver((muts) => {
      let newFound = false;
      muts.forEach(m => {
        m.addedNodes.forEach(n => { if (n.nodeType === 1 && n.matches && n.matches('.gallery-card')) newFound = true; });
      });
  ...
  (function setupInfiniteScroll() {
    ...
    const infiniteObserver = new IntersectionObserver(entries => {
      entries.forEach(entry => {
        ...
        const loadMoreBtn = document.querySelector('#load-more-btn, .load-more-btn, button.load-more, .btn-load-more');
        if (loadMoreBtn) {
          // small guard: avoid double-clicking while loading
          const isDisabled = loadMoreBtn.disabled || loadMoreBtn.classList.contains('loading');
          if (!isDisabled) {
            loadMoreBtn.click();
          }
        }
