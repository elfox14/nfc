document.addEventListener("DOMContentLoaded", () => {
  ...
  const getVCardString = () => {
    ...
    const nameParts = (inputs["input-name"] || "")
      .replace(/\\n/g, " ")
      .split(" ");
    ...
    if (dynamicData.phones) {
      const whatsappNumberClean =
        staticSocial.whatsapp && staticSocial.whatsapp.value
  ...
  const renderContactLinks = (data) => {
    ...
    const createLinkHTML = (key, value) => {
      ...
      if (key === "email") {
        ...
        // For dynamic social links and others
        fullUrl = !/^(https?:\/\/)/i.test(value) ? `${prefix}${value}` : value;
        displayValue = value.replace(/^(https?:\/\/)?(www\.)?/, "");
      }
    ...
    if (dynamicData.phones) {
      ...
      dynamicData.phones.forEach((phone) => {
        if (phone && phone.value) {
          const phoneValueClean = phone.value.replace(/\D/g, "");
          if (phoneValueClean !== whatsappNumberClean) {
            const sanitizedValue = phone.value
              .replace(/</g, "&lt;")
              .replace(/>/g, "&gt;");
            linksHTML.push(`
    ...
    if (linksHTML.length > 0) {
      ...
      container.innerHTML =
        '<p style="text-align: center; color: var(--text-secondary);">لا توجد روابط متاحة</p>';
    }
  ...
  const buildCardForRender = async (data) => {
    ...
    fontAwesomeLink.href =
      "https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css"; // Use a specific version known to work
    ...
    const renderElement = (elementHTML, placement, containerCollection) => {
      if (containerCollection[placement]) {
        containerCollection[placement].insertAdjacentHTML(
          "beforeend",
          elementHTML,
        );
      } else {
    ...
    const frontOpacity =
      inputs["front-bg-opacity"] !== undefined ? inputs["front-bg-opacity"] : 1;
    const backOpacity =
      inputs["back-bg-opacity"] !== undefined ? inputs["back-bg-opacity"] : 1;
    ...
    if (inputs["input-logo"]) {
      ...
      const logoOpacity =
        inputs["logo-opacity"] !== undefined ? inputs["logo-opacity"] : 1;
    ...
    if (inputs["input-photo-url"]) {
      ...
      const photoBorderWidth =
        inputs["photo-border-width"] !== undefined
          ? inputs["photo-border-width"]
          : 2;
    ...
    if (inputs["input-name"]) {
      ...
      const displayName = (inputs["input-name"] || "")
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;");
      const nameHTML = `
    ...
    if (inputs["input-tagline"]) {
      ...
      // Basic sanitization for display
      const displayTagline = (inputs["input-tagline"] || "")
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;");
      const taglineHTML = `
    ...
    if (dynamicData.phones && dynamicData.phones.length > 0) {
      const showAsButtons =
        inputs["toggle-phone-buttons"] !== undefined
          ? inputs["toggle-phone-buttons"]
          : true; // Default true
      ...
      const phoneBtnPadding =
        inputs["phone-btn-padding"] !== undefined
          ? inputs["phone-btn-padding"]
          : 6;
    ...
    const platforms = {
      /* Copy the platforms object from renderContactLinks here */
      whatsapp: { icon: "fab fa-whatsapp", prefix: "https://wa.me/" },
      email: { icon: "fas fa-envelope", prefix: "mailto:" },
      website: { icon: "fas fa-globe", prefix: "https://" },
      facebook: { icon: "fab fa-facebook-f", prefix: "https://facebook.com/" },
    ...
    if (allSocialLinks.length > 0) {
      const showSocialButtons =
        inputs["toggle-social-buttons"] !== undefined
          ? inputs["toggle-social-buttons"]
          : true; // Default true
      ...
      const socialBtnFont =
        inputs["back-buttons-font"] || "Poppins, sans-serif";
      const socialTextSize = inputs["social-text-size"] || 12;
      const socialTextColor = inputs["social-text-color"] || "#e6f0f7";
      const socialTextFont =
        inputs["social-text-font"] || "Tajawal, sans-serif";
      ...
      allSocialLinks.forEach((link) => {
        ...
        if (link.platformKey === "email") {
          fullUrl = `${prefix}${value}`;
        } else if (link.platformKey === "whatsapp") {
          fullUrl = `${prefix}${value.replace(/\D/g, "")}`;
        } else {
    ...
    if (qrSource === "custom") {
      ...
      console.warn(
        "auto-card QR source selected for rendering, using vCard data as fallback.",
      );
      qrDataString = getVCardString();
    }
    ...
    if (qrDataString) {
      ...
      if (qrSource === "custom" || qrSource === "upload") {
        ...
        // Check if vCard/auto-card data is substantial
          ...
          await new Promise((resolve) =>
            setTimeout(() => {
              ...
              if (dataUrl) {
                ...
                console.warn(
                  "QR Code generation did not produce an image or canvas.",
                );
              }
  ...
  const captureAndDisplayCards = async () => {
    ...
    try {
      ...
      const backImageStyle = isMobileView
  ...
  const processCardData = async (data) => {
    ...
    try {
      ...
      if (capturedFront && capturedBack) {
        ...
        console.warn(
          "Captured images not found in data, attempting to render and capture...",
        );
  ...
  const addSaveButtonListeners = () => {
      ...
      saveVcfBtn.onclick = () => {
        try {
          ...
          if (!vcfData || vcfData.length < 20) {
            ...
            console.warn(
              "Attempted to save VCF with insufficient data:",
              vcfData,
            );
            return;
          }
          ...
          link.href = url;
          const filenameBase = (
            cardData && cardData.inputs && cardData.inputs["input-name"]
    ...
    const downloadCapturedImage = (cardFace) => {
      ...
      if (
        imgElement &&
        imgElement.src &&
        (imgElement.src.startsWith("data:image/png") ||
        ...
        try {
          ...
          alert(
            `حدث خطأ أثناء تجهيز صورة الواجهة ${cardFace === "front" ? "الأمامية" : "الخلفية"}.`,
          );
        }
      } else {
        console.warn(
          `Captured image element for '${cardFace}' not found or has invalid src.`,
        );
        alert(
          `لم يتم العثور على صورة البطاقة ${cardFace === "front" ? "الأمامية" : "الخلفية"} أو أنها غير صالحة.`,
        );
      }
    };
    ...
    if (savePdfBtn) {
      savePdfBtn.onclick = async () => {
        ...
        const frontImgElement = document
          .getElementById("card-front-display")
          ?.querySelector("img");
        const backImgElement = document
        ...
        if (
          ...
          alert(
            "لم يتم العثور على صور البطاقة الأمامية والخلفية أو أنها غير صالحة لإنشاء PDF.",
          );
  ...
  const initializeViewer = async () => {
    ...
    try {
      ...
      if (
        ...
        let relevantSegments = pathSegments.filter(
          (p) => p.toLowerCase() !== "viewer.html",
        );
        ...
        if (!cardId) {
          throw new Error(
            "لم يتم العثور على معرف البطاقة في الرابط. يرجى فتح رابط مشاركة صالح.",
          );
        }
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `فشل تحميل بيانات البطاقة (الحالة: ${response.status})`,
          );
        }
      ...
      showLoadingError(
        error.message || "حدث خطأ غير متوقع أثناء تهيئة العارض.",
      );
    }
  };
