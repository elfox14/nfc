...
describe('production release verifier', () => {
  test('derives current Brand Kit and workspace assets', () => {
    expect(verifier.extractExpectedRelease(rootDir)).toBe('2026.07.20-phase15.0');
    expect(verifier.extractExpectedServiceWorkerCache(rootDir)).toBe('v19');
  ...
  test('detects stale runtime files', async () => {
    const stale = read('runtime-config.js').replace('2026.07.20-phase15.0', '2026.07.20-phase13.1');
    const report = await verifier.verifyProduction({
      rootDir,
