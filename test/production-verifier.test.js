...
describe('production release verifier', () => {
  test('derives current Brand Kit and workspace assets', () => {
    expect(verifier.extractExpectedRelease(rootDir)).toBe('2026.07.20-phase13.1');
    expect(verifier.extractExpectedServiceWorkerCache(rootDir)).toBe('v17');
  ...
  test('passes when production matches the repository', async () => {
    ...
    expect(report.expected).toMatchObject({
      release: '2026.07.20-phase13.1',
      serviceWorkerCache: 'v17',
  ...
  test('detects stale runtime files', async () => {
    const stale = read('runtime-config.js').replace('2026.07.20-phase13.1', '2026.07.20-phase12.0');
    const report = await verifier.verifyProduction({
      rootDir,
