...
describe('production release verifier', () => {
  test('derives current Brand Kit and workspace assets', () => {
    expect(verifier.extractExpectedRelease(rootDir)).toBe('2026.07.20-phase14.0');
    expect(verifier.extractExpectedServiceWorkerCache(rootDir)).toBe('v18');
  ...
  test('passes when production matches the repository', async () => {
    ...
    expect(report.totals).toEqual({ checks: 29, passed: 29, failed: 0 });
    expect(fetchImpl).toHaveBeenCalledTimes(29);
    expect(report.expected).toMatchObject({
      release: '2026.07.20-phase14.0',
      serviceWorkerCache: 'v18',
  ...
  test('detects stale runtime files', async () => {
    const stale = read('runtime-config.js').replace('2026.07.20-phase14.0', '2026.07.20-phase13.1');
    const report = await verifier.verifyProduction({
      rootDir,
