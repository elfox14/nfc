...
describe('production release verifier', () => {
  test('derives current Brand Kit and workspace assets', () => {
    expect(verifier.extractExpectedRelease(rootDir)).toBe('2026.07.22-phase22');
    expect(verifier.extractExpectedServiceWorkerCache(rootDir)).toBe('v27');
  ...
  test('passes when production matches the repository', async () => {
    ...
    expect(report.expected).toMatchObject({
      release: '2026.07.22-phase22',
      serviceWorkerCache: 'v27',
  ...
  test('detects stale runtime files', async () => {
    const stale = read('runtime-config.js').replace('2026.07.22-phase22', '2026.07.20-phase13.1');
    const report = await verifier.verifyProduction({
      rootDir,
