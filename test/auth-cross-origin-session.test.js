...
function loadAuth(fetchImpl) {
  ...
  const window = {
    location: {
      origin: 'https://mcprim.com',
      hostname: 'mcprim.com',
      pathname: '/nfc/dashboard.html'
    },
...
describe('Cross-origin OAuth session fallback', () => {
  it('stores the session-init access token only for the current browser tab', async () => {
  ...
  it('clears the tab-scoped bearer token on logout state cleanup', () => {
