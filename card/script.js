    ...
    const ShareManager = {
        async saveDesign() {
            ...
            try {
                ...
                if (!token) {
                    alert('يجب تسجيل الدخول أولاً لحفظ التصميم. / You must be logged in to save a design.');
                    return null;
                }
        ...
        async shareCard() {
            ...
            const token = localStorage.getItem('authToken');
            if (!token) {
                alert('يجب تسجيل الدخول أولاً لمشاركة بطاقتك. / You must be logged in to share your card.');
                return;
            }
