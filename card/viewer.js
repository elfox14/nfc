document.addEventListener('DOMContentLoaded', () => {
    ...
    const setupEventListeners = () => {
        document.getElementById('save-front-png-btn').addEventListener('click', () => captureAndDownload(document.getElementById('card-front-preview'), 'card-front.png'));
        document.getElementById('save-back-png-btn').addEventListener('click', () => captureAndDownload(document.getElementById('card-back-preview'), 'card-back.png'));
    ...
    const captureAndDownload = async (element, filename, scale = 2) => {
        try {
            const canvas = await html2canvas(element, { backgroundColor: null, scale, useCORS: true });
    ...
    const buildBackCardContent = (data) => {
        ...
        const renderLink = (value, platform) => {
            if (!value) return '';
            let fullUrl = value;
            if (platform.prefix && !value.startsWith('http') && !value.startsWith('mailto:')) {
        ...
        linksHtml += renderLink(data.inputs['input-website'], socialPlatforms.website);
        linksHtml += renderLink(data.inputs['input-whatsapp'], socialPlatforms.whatsapp);
        linksHtml += renderLink(data.inputs['input-facebook'], socialPlatforms.facebook);
        linksHtml += renderLink(data.inputs['input-linkedin'], socialPlatforms.linkedin);
