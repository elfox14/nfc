    ...
    init() {
        ...
        const steps = [
            ...
            },
            {
                ...
                attachTo: { element: isMobile ? '.pro-mobile-nav .mobile-tab-btn[data-tab-target="#panel-elements"]' : '.pro-sidebar-right', on: isMobile ? 'top' : 'left' },
                beforeShowPromise: () => {
                    if (isMobile) {
                        // التأكد من أننا في التبويب الصحيح
                        document.querySelector('.pro-mobile-nav .mobile-tab-btn[data-tab-target="#panel-elements"]').click();
                    }
            ...
            },
            {
                ...
                attachTo: { element: isMobile ? '.pro-mobile-nav .mobile-tab-btn[data-tab-target="#panel-design"]' : '.pro-sidebar-left', on: isMobile ? 'top' : 'right' },
                 beforeShowPromise: () => {
                    if (isMobile) {
                        // الانتقال للتبويب التالي
                        document.querySelector('.pro-mobile-nav .mobile-tab-btn[data-tab-target="#panel-design"]').click();
                    }
                    return Promise.resolve();
                }
            },
...
const UIManager = {
    ...
    navigateToAndHighlight(elementId) {
        ...
        if (isMobile && parentPane && !parentPane.classList.contains('active')) {
            ...
            if (buttonToClick) {
                const allButtons = document.querySelectorAll('.pro-mobile-nav .mobile-tab-btn');
