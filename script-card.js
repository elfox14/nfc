...
const DragManager = {
    ...
    dragMoveListener(event) {
        ...
        // Smart Snapping Logic (Phase 1)
        const snapThreshold = 10; // pixels
        const cardContainer = target.closest('.business-card');
        ...
        if (cardContainer) {
            // Get bounding rects to calculate centers relative to the page
            const containerRect = cardContainer.getBoundingClientRect();
            // We simulate the new position to see where its center WOULD be
            // getBoundingClientRect is read-only, so we calculate the offset manually.
            const targetRect = target.getBoundingClientRect();

            // The x/y data attributes translate the element from its ORIGINAL DOM position.
            // Center of the container in the viewport:
            const containerCenterX = containerRect.left + (containerRect.width / 2);
            const containerCenterY = containerRect.top + (containerRect.height / 2);

            // New expected Center of the target element in the viewport:
            const targetCenterX = targetRect.left + dx + (targetRect.width / 2);
            const targetCenterY = targetRect.top + dy + (targetRect.height / 2);

            // X-Axis Snapping (Vertical Guide)
            if (Math.abs(targetCenterX - containerCenterX) < snapThreshold) {
                // Adjust x so the centers match
                x -= (targetCenterX - containerCenterX);
                if (guideV) {
                    guideV.style.display = 'block';
                    // Position the guide exactly in the middle of the card container
                    guideV.style.left = (cardContainer.offsetLeft + cardContainer.offsetWidth / 2) + 'px';
                    guideV.style.top = cardContainer.offsetTop + 'px';
                    guideV.style.height = cardContainer.offsetHeight + 'px';
                }
            }

            // Y-Axis Snapping (Horizontal Guide)
            if (Math.abs(targetCenterY - containerCenterY) < snapThreshold) {
                // Adjust y so the centers match
                y -= (targetCenterY - containerCenterY);
                if (guideH) {
                    guideH.style.display = 'block';
                    // Position the guide exactly in the middle of the card container
                    guideH.style.top = (cardContainer.offsetTop + cardContainer.offsetHeight / 2) + 'px';
                    guideH.style.left = cardContainer.offsetLeft + 'px';
                    guideH.style.width = cardContainer.offsetWidth + 'px';
                }
            }
        }
...
const CardManager = {
    ...
    updateElementFromInput(input) {
        let { updateTarget, updateProperty, updateUnit = '' } = input.dataset;
        if (!updateTarget || !updateProperty) return;
        const targetElement = document.getElementById(updateTarget);
        if (!targetElement) return;

        // Auto-detect unit for specific CSS properties (Phase 3 enhancement)
        if (!updateUnit && (updateProperty.endsWith('letterSpacing') || updateProperty.endsWith('borderRadius'))) {
            updateUnit = 'px';
        }
    ...
    updateTextGlow(elementId) {
        const enabled = document.getElementById(`${elementId}-text-glow-enabled`)?.checked;
        const color = document.getElementById(`${elementId}-text-glow-color`)?.value || '#000000';
        const blur = document.getElementById(`${elementId}-text-glow-blur`)?.value || '4';
        const targetElement = DOMElements.draggable[elementId];

        if (targetElement) {
            targetElement.style.textShadow = enabled ? `0 0 ${blur}px ${color}` : 'none';
        }
    },

    updateTextTransform(elementId) {
        const isUppercase = document.getElementById(`${elementId}-uppercase`)?.checked;
        const targetElement = DOMElements.draggable[elementId];

        if (targetElement) {
            targetElement.style.textTransform = isUppercase ? 'uppercase' : 'none';
        }
    },
    ...
    updatePhoneButtonStyles() {
        ...
        const styleSelect = document.getElementById('phone-btn-style')?.value || 'solid';
        const radius = document.getElementById('phone-btn-radius')?.value || 8;
        ...
        document.querySelectorAll('.phone-button').forEach(button => {
            ...
            // Apply Advanced Button Styles (Phase 3)
            if (styleSelect === 'outline') {
                button.style.backgroundColor = 'transparent';
                button.style.border = `2px solid ${bgColor}`;
                button.style.color = textColor;
            } else if (styleSelect === 'glass') {
                button.style.backgroundColor = `${bgColor}40`; // 25% opacity assuming hex color
                ...
                button.style.color = textColor;
                button.style.border = (bgColor === 'transparent' || bgColor.includes('rgba(0,0,0,0)')) ? `1px solid ${textColor}` : '1px solid transparent';
            }
