...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const generateEmailSignature = () => {
    ...
    const photoUrl =
      cardData.imageUrls.photo || "https://via.placeholder.com/80";
  ...
  const renderContactLinks = (data) => {
    ...
    const showSocial =
      inputs["toggle-master-social"] === undefined ||
      inputs["toggle-master-social"] === true;
    if (!showSocial) {
      container.innerHTML = `
  ...
  const buildCardForRender = async (data) => {
    ...
    const getPlacement = (key, defaultPlacement = "front") =>
      placements[key] || defaultPlacement;
    ...
    if (showSocial) {
      ...
      dynamicSocial.forEach((linkData) => {
        if (
          ...
          allSocialLinks.push({
            id: linkData.id || `dynamic-${linkData.platform}-${Date.now()}`,
            value: linkData.value,
            placement: linkData.placement || "back",
            position: linkData.position || { x: 0, y: 0 },
    ...
    } else if (qrSource === "auto-vcard" || qrSource === "auto-card") {
  ...
  const processCardData = async (data) => {
    ...
    try {
      ...
      if (isVertical) {
        ...
        if (frontRenderWrapper)
          frontRenderWrapper.classList.remove("is-vertical");
        if (backRenderWrapper)
          backRenderWrapper.classList.remove("is-vertical");
      }
      // --- [END] ---
    ...
    const downloadCapturedImage = (cardFace) => {
      ...
      if (
        ...
        try {
          ...
          alert(`حدث خطأ أثناء تجهيز صورة الواجهة.`);
        }
      } else {
        alert(`لم يتم العثور على صورة البطاقة.`);
      }
    };
