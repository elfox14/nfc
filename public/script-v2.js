...
class EditorV2 {
    ...
    initInteraction() {
        ...
        const container = document.getElementById('v2-card-canvas');
        ...
        selectionLayer.addEventListener('mousedown', (e) => {
            ...
            isResizing = true;
            currentHandle = handle.getAttribute('data-handle-type');
            targetEl = this.getSelectedElementData();
            if (!targetEl) return;

            startX = e.clientX;
            startY = e.clientY;
            originalTransform = { ...targetEl.transform };
            e.preventDefault();
            e.stopPropagation();
        });

        document.getElementById('canvas-elements-layer').addEventListener('mousedown', (e) => {
        ...
        window.addEventListener('mousemove', (e) => {
            if ((!isDragging && !isResizing) || !targetEl) return;

            const dx = (e.clientX - startX) / this.zoom;
            const dy = (e.clientY - startY) / this.zoom;

            if (isDragging) {
                let newX = originalTransform.x + dx;
                let newY = originalTransform.y + dy;
                if (this.isGridActive) {
                    newX = Math.round(newX / this.snapSize) * this.snapSize;
                    newY = Math.round(newY / this.snapSize) * this.snapSize;
                }
                targetEl.transform.x = newX;
                targetEl.transform.y = newY;
            } else if (isResizing) {
                this.handleResize(targetEl, currentHandle, dx, dy, originalTransform);
            }
